...
func printTest(s junit.Testsuite, c junit.Testcase, result *junit.Result, isFailure bool) {
	...
	titleFormat, titleArgs := func() (string, []any) {
		if boostCompatible {
			return "<a href='#%s'>%s / %s <span class='badge'>%s</span></a>\n", []any{id, c.Classname, c.Name, text}
		}

		return "<a href='#%s'>%s <span class='badge'>%s</span></a>\n", []any{id, c.Name, text}
	}()

	fmt.Fprintf(outputFile, titleFormat, titleArgs...)
...
func init() {
	flag.BoolVar(&boostCompatible, "boost_compatible", false, "Parse report format generated by Boost")
